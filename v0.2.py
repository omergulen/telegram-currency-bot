...
async def on_callback_query(msg):
    ...
    print(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),'Callback query:', query_id, from_id, msg['from']['username'] , data)
    log = 'Callback query: ' + str(query_id)+ " " + str(from_id) + " " + msg['from']['username'] + " " + data
    logging.info(log)

    if data == '/hidekeyboard':
        markup = ReplyKeyboardRemove()
        await bot.sendMessage(chat_id, '/start to re-open keyboard\n'
                                       '/about to advice us :)', reply_markup=markup)
