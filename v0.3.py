...
def on_chat_message(msg):
    ...
    print(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),'Chat:',  content_type, chat_type, chat_id, msg['from']['username'], msg['text'])
    log = 'Chat: ' + content_type + " " + chat_type + " " +str(chat_id) + " " + msg['from']['username'] +" "+ msg['text']
    logging.info(log)
    ...
    if msg['text'] == "/start":
        ...
        bot.sendMessage(chat_id, 'Select currency to get information about.', reply_markup=markup)
        ...
        bot.sendMessage(chat_id, "BitCoin @ {}\n"
            ...
            "BTC % Changed in 24h : {} %".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        ...
        bot.sendMessage(chat_id, "Etherium Classic @ {}\n"
            "1 ETC = {:.2f} USD\n"
            "1 ETC = {:.2f} EUR\n"
            "1 ETC = {:.2f} TRY\n"
            "ETC-USD Volume in 24h : {} USD\n"
            "ETC % Changed in 24h : {} %".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        ...
        bot.sendMessage(chat_id, "Ripple @ {}\n"
            "1 XRP = {:.2f} USD\n"
            "1 XRP = {:.2f} EUR\n"
            "1 XRP = {:.2f} TRY\n"
            "XRP-USD Volume in 24h : {} USD\n"
            "XRP % Changed in 24h : {} %".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        ...
        bot.sendMessage(chat_id, "Ethereum @ {}\n"
            "1 ETH = {:.2f} USD\n"
            "1 ETH = {:.2f} EUR\n"
            "1 ETH = {:.2f} TRY\n"
            "ETH-USD Volume in 24h : {} USD\n"
            "ETH % Changed in 24h : {} %".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        ...
        bot.sendMessage(chat_id, "Litecoin @ {}\n"
            "1 LTC = {:.2f} USD\n"
            "1 LTC = {:.2f} EUR\n"
            "1 LTC = {:.2f} TRY\n"
            "LTC-USD Volume in 24h : {} USD\n"
            "LTC % Changed in 24h : {} %".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        ...
        bot.sendMessage(chat_id, "Dash @ {}\n"
           "1 DASH = {:.2f} USD\n"
           "1 DASH = {:.2f} EUR\n"
           "1 DASH = {:.2f} TRY\n"
           "DASH-USD Volume in 24h : {} USD\n"
           "DASH % Changed in 24h : {} %".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        ...
        bot.sendMessage(chat_id, "Monero @ {}\n"
           "1 XMR = {:.2f} USD\n"
           "1 XMR = {:.2f} EUR\n"
           "1 XMR = {:.2f} TRY\n"
           "XMR-USD Volume in 24h : {} USD\n"
           "XMR % Changed in 24h : {} %".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        ...
        bot.sendMessage(chat_id,"To advice us:\n"
                                      "https://github.com/omergulen/telegram-currency-bot\n"
                                      "or\n"
                                      "omrglen@gmail.com\n"
                                      "Thanks for support!")
        bot.sendMessage(chat_id, '/start to re-open keyboard')
